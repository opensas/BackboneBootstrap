...
(function ($, undefined) {
    ...
    AbstractSelect2 = clazz(Object, {
        ...
        positionDropdown: function() {
            var rect = $('.select2-container.bigdrop.select2-dropdown-open')[0];
						if (!rect) return;
						rect=rect.getClientRects()[0];
						var offset = this.container.offset(),
                height = this.container.outerHeight(),
                width = this.container.outerWidth(),
                rect = $('.select2-container.bigdrop.select2-dropdown-open')[0].getClientRects()[0],//offset.top + height,rect
								width = rect.width,
        ...
        ensureHighlightVisible: function () {
            ...
            child = $(children[index]);
						...
						hhb=child.outerHeight();
						if (isNaN(hhb))
							hhb=child[0].clientHeight;
            hb = child.offset().top + hhb;

            // if this is the last child lets also make sure select2-more-results is visible
            if (index === children.length - 1) {
                more = results.find("li.select2-more-results");
                if (more.length > 0) {
									mmb=more.outerHeight();
									if (isNaN(mmb))
										mmb=more[0].clientHeight;
									hb = more.offset().top + mmb;
                }
            }
						rrb=results.outerHeight();
						if (isNaN(rrb))
							rrb=results[0].clientHeight;

            rb = results.offset().top + rrb;
            if (hb > rb) {
